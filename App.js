...
export default function App() {
  ...
  const gerarMegaSena = () => {
    ...
    while (numeros.length != 6) {
      // verifica se tem os 6 n. caso nao continua o codigo
      numeros.push(Math.floor(Math.random() * 60) + 1); // gera n entre 1 e 60

      numeros = numeros.filter((item, index) => {
        // verifica se tem repeticao nos numeros gerados
  ...
  return (
    <View style={styles.container}>
      <View style={styles.cardView}>
        ...
        <Button title="Gerar números da MEGA SENA" onPress={gerarMegaSena} />
...
const styles = StyleSheet.create({
  ...
  cardView: {
    ...
    borderRadius: 10,
    boxShadow: "inset 0 0 1em silver, 0 0 1em #6c2a6a",
  }
});
